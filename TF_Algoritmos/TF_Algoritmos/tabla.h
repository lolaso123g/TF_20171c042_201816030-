...
class Table {
	...
	// ordenamiento merge para cuando no hay indexado en la columna
	void merge(vector<Fila*>& oFilas, int ini, int fin, int numCol) {
		...
		int len = (fin - ini) + 1;
		vector<Fila*> temp = oFilas;
		int mid = (fin + ini) / 2;
		int i = ini;
		int j = mid + 1;

		if (columnas[numCol - 1]->getTipo() == 1) {
			for (int k = 0; i <= mid || j <= fin; k++) {
				if (i > mid || j <= fin && k1(oFilas[j], numCol) < k1(oFilas[i], numCol)) temp[k] = oFilas[j++];
				else temp[k] = oFilas[i++];
			}
		}
		else if (columnas[numCol - 1]->getTipo() == 2) {
			for (int k = 0; i <= mid || j <= fin; k++) {
				if (i > mid || j <= fin && k2(oFilas[j], numCol) < k2(oFilas[i], numCol)) temp[k] = oFilas[j++];
				else temp[k] = oFilas[i++];
			}
		}
		else if (columnas[numCol - 1]->getTipo() == 3) {
			for (int k = 0; i <= mid || j <= fin; k++) {
				if (i > mid || j <= fin && k3(oFilas[j], numCol) < k3(oFilas[i], numCol)) temp[k] = oFilas[j++];
				else temp[k] = oFilas[i++];
			}
		}
		for (int k = 0; k < len; k++) {
	...
	void sort(vector<Fila*>& oFilas, int i, int f, int numCol) {
		if (i < f) {
			int mid = (i + f) / 2;
			sort(oFilas, i, mid, numCol);
			sort(oFilas, mid + 1, f, numCol);
			merge(oFilas, i, f, numCol);
		}
	};
	void mergeSort(vector<Fila*>& enOrden, int n, int numCol) {
		sort(enOrden, 0, n - 1, numCol);
	}
	//////////////////////////////////////////////////////////////////////////////
public:
	Table() {
	...
	void addFila() {
		...
		for (int i = 0; i < columnas.size(); i++) {
			...
			// chequeo si hay una rbol existente para la columna, si existe se agrega la nueva fila
			for (int j = 0; j < arbolesInt.size(); j++) {
				if (arbolesInt[j]->getNColumna() == i + 1) {
					arbolesInt[j]->add(new Fila(cantFilas));
					break;
				}
			}
			for (int j = 0; j < arbolesStr.size(); j++) {
				if (arbolesStr[j]->getNColumna() == i + 1) {
					arbolesStr[j]->add(new Fila(cantFilas));
					break;
				}
			}
			for (int j = 0; j < arbolesFlt.size(); j++) {
				if (arbolesFlt[j]->getNColumna() == i + 1) {
					arbolesFlt[j]->add(new Fila(cantFilas));
					break;
				}
			}
		}
	...
	vector<Fila*> ordenar() {
		...
		if (!arbol) {
			enOrden = filas;
			mergeSort(enOrden, filas.size(), numColumna);
		}
		return enOrden;
	}
	void slowFiltr(int numColumna, int tipoF, string valor) { // filtrado secuencial
		if (tipoF == 1) {
			if (filtro1.empty()) {
	...
	void filtrar() { // se busca si la columna a filtrar esta indexada, si es el caso, se hace el filtrado con los metodos de la clase arbol
					 // caso contrario se hace un filtrado secuencial en el metodo slowFiltr arriba
		bool arbol = false;		// bool que indica si se encontro un indexado o no	
		int numColumna = 0;
		string valor;
		char opcionFiltro;
		do {
		...
		cout << "Si ya ha ingresado dos filtros se va a sobrescribir el ultimo !" << endl;
	...
	void mostrarFiltro() { // imprime la interseccion de ambos vectores de filtros 
		print = [&](Fila* a) {a->printFila(columnas); };
	...
	void exportarArch() {
		...
		if (archivo.fail()) {
			...
			cout << "Se exportaron los datos al archivo exitosamente" << endl;
			system("pause");
		}
		archivo.close();
	}

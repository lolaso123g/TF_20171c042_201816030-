...
int main() {
	...
	vector<Table*> tablas;
	int nCol, idxT, tempidx, confirmTabla;
	vector<int> numC;
	...
	do
	{
		...
		case '9':
			...
			do {
				cout << "Tiene guardadas " << tablas.size() << "tablas" << endl;
			...
			tablas[tempidx - 1]->mostrar();
			do {
				cout << "Confirmar eleccion de tabla? (1:Si , 0:No): ";
				cin >> confirmTabla;
			} while (confirmTabla < 0 || confirmTabla > 1);
			if (confirmTabla == 1) { idxT = tempidx - 1; }
			break;
		case '0':
			bandera = true;
